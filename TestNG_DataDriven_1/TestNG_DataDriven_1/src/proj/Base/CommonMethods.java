...
public class CommonMethods {
	...
	public void handelCaptcha(String message) throws Exception
	{
		String CaptchaSendkeys = Setup.OR.getProperty("txtboxCaptcha");
		
		for(int i=0;i<=i+1;i++)
		{
		String input = JOptionPane.showInputDialog(null,message +" :" + "\n" + "To Reload Enter 1 :");
		WebPage.windowHandle();
		System.out.println("the input value is:" + input);
		WebPage.sendKeys(CaptchaSendkeys, "X", input);
		try
		{
		if(!input.contentEquals("1"))
		{
		WebPage.switchToDefaultConatiner();
		WebPage.click("btnRegisterNow", "id");
		break;
		}
		else
		{
			System.out.println("The loop entered into else");
			WebPage.click("recaptcha_reload", "id");
		}
		}
		catch(Exception e)
		{
